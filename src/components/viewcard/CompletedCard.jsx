...
const InprocessCard = ({ tasks, setTasks, updatedTasks, result }) => {
    ...
    return (
        <div className="border divide-y rounded-lg overflow-hidden max-w-6xl mt-4 mb-10 mx-auto">
            <div>
                <div className="justify-between w-full text-base text-left py-4 px-6 text-black bg-[#CEFFCC] flex transition-all items-center">
                    <span className="mr-4">
                ...
                <div className={`${isopen ? "block" : "hidden"} py-4 px-6 min-h-[30vh] flex flex-col bg-gray-50`}>
                    {
                        result && result.length > 0 ? renderTasks(result.filter(data => data.TaskStatus === "Completed")) :
                            updatedTasks && updatedTasks.length > 0 ? renderTasks(updatedTasks.filter(task => task.TaskStatus === "Completed")) :
